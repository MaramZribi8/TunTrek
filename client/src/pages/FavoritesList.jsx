...
export default function FavoritesList() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const [userResponse, placesResponse] = await Promise.all([
          axios.get('/profile'), // This fetches the complete user profile
          axios.get('/places')
        ]);
        setUser(userResponse.data); // Set user with favorites extracted from the profile
    ...
    fetchData();
  }, [setUser]); // Remove setUser from dependencies, it's a setter and doesn't need to trigger effects
  ...
  const toggleFavorite = async (placeId) => {
    ...
    try {
      ...
      setFavorites(updatedFavorites); // Ensure local state is updated after API call
    } catch (error) {
      ...
      {isLoading ? (
        ...
        favorites.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {places.filter(place => favorites.includes(place._id) && isStillAvailable(place)).map((place) => (
